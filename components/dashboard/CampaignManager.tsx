...
export default function CampaignManager({ leadId, initialItems }: Props) {
    ...
    // Auto-search when selected districts change
    useEffect(() => {
        if (selectedDistricts.length > 0) {
            ...
            // Keep menu open when selecting multiple
            // fetch with both state and districts
            ...
            // If no districts selected, maybe clear results or show state-wide?
            // For now, let's clear results to encourage selection
            // setSearchResults([]) 
            // setIsMenuOpen(false)
        }
    ...
    const addAll = async () => {
        ...
        setAdding(-1) // Special indicator for bulk add
        try {
            const res = await fetch(`/api/leads/${leadId}/campaign-items`, {
                method: "POST",
                body: JSON.stringify({ inventoryIds: availableResults.map(i => i.id) })
            })
            if (!res.ok) throw new Error("Failed")
            router.refresh()
            setTimeout(() => window.location.reload(), 500)
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-sm border space-y-4">
            ...
            <div className="relative z-10">
                ...
                {isMenuOpen && (searchQuery.length > 2 || searchResults.length > 0) && (
                    <div className="absolute top-full mt-1 w-full bg-white border rounded-lg shadow-xl ring-1 ring-black/5 z-20">
                        {availableResults.length > 0 && (
                            <div className="p-2 border-b bg-gray-50 flex justify-between items-center">
                                <span className="text-xs text-gray-500 font-medium ml-1">{availableResults.length} items found</span>
                                <button
                                    onClick={addAll}
                                    disabled={adding === -1}
                                    className="text-xs font-bold text-blue-600 hover:text-blue-800 hover:underline px-2 py-1 flex items-center gap-1"
                                >
                                    {adding === -1 && <Loader2 className="w-3 h-3 animate-spin" />}
                                    Add All
                                </button>
                            </div>
                        )}
                                ...
                                availableResults.map(res => (
                                    <div key={res.id} className="p-2.5 hover:bg-blue-50/50 flex justify-between items-center border-b last:border-0 cursor-pointer transition-colors group" onClick={() => addItem(res)}>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Button
                                                size="sm"
                                                disabled={adding === res.id || adding === -1}
            ...
            <div className="space-y-3">
                ...
                {/* Districts in Selected State - Enhanced Card with Multi-Select and Search */}
                {selectedState && (
                                        ...
                                        {filteredDistricts.map(({ district, count }) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full px-4 py-3 hover:bg-blue-50 text-left transition-colors group flex items-center justify-between ${isSelected ? 'bg-blue-50/80' : ''}`}
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className={`text-sm font-semibold transition-colors ${isSelected ? 'text-blue-700' : 'text-gray-900'}`}>
                                                                {district}
                                                            </div>
                                                    ...
                                                    <div className={`transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                                        {isSelected ? (
                                                            <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider bg-blue-100 px-2 py-0.5 rounded-full">Selected</span>
                                                        ) : (
